// I18n messages for TypeScript

/** Translated messages */
export const I18N_MESSAGES = {
    "About":
    "Acerca de",

    "Advanced search":
    "Búsqueda avanzada",

    "All posts":
    "Todos los artículos",

    "Any changes you made to the form will be lost.":
    "Cualquier cambio que hay realizado en el formulario se perderá.",

    "Any time":
    "En cualquier momento",

    "Author editor":
    "Edición de autores",

    "Black":
    "Negro",

    "Blue":
    "Azul",

    "Cancel":
    "Cancelar",

    "Catalogs":
    "Catálogos",

    "Commented":
    "Comentados",

    "Commented articles":
    "Artículos comentados",

    "Create project":
    "Crear proyecto",

    "Create school":
    "Crear centro",

    "Create synapse":
    "Crear sinapsis",

    "Cyan":
    "Cian",

    "Default ordering":
    "Ordenación predeterminada",

    "Delete":
    "Suprimir",

    "Delete comment":
    "Suprime el comentario",

    "Delete synapse":
    "Suprime la sinapsis",

    "Delete tag":
    "Suprime la etiqueta",

    "Disable user":
    "Deshabilitar el usuario",

    "Do you want to delete «<strong>{name}</strong>»?":
    "¿Seguro que quieres suprimir  «<strong>{name}</strong>»?",

    "Do you want to delete this block?":
    "¿Quieres suprimir este bloque?",

    "Do you want to disable the user «<strong>{name}</strong>»?":
    "¿Seguro que quieres desactivar el usuario  «<strong>{name}</strong>»?",

    "Do you want to reset the form to its initial state?":
    "¿Desea restablecer el formulario a su estado inicial?",

    "Educators":
    "Docentes",

    "Everything":
    "Todos",

    "Favourited":
    "Favoritos",

    "Favourited articles":
    "Artículos favoritos",

    "Featured":
    "Destacados",

    "Featured articles":
    "Artículos destacados",

    "Gray":
    "Gris",

    "Green":
    "Verde",

    "Help":
    "Ayuda",

    "Leave this page":
    "Salir de la página",

    "Liked":
    "Votados",

    "Liked articles":
    "Artículos votados",

    "Most commented":
    "Más comentados",

    "Most commented articles":
    "Artículos más comentados",

    "Most recent articles first":
    "Los artículos más recientes primero",

    "Most relevant results first":
    "Los resultados más relevantes primero",

    "Most voted":
    "Más votados",

    "Most voted articles":
    "Artículos más votados",

    "Municipalities":
    "Municipios",

    "Name of the project":
    "Nombre del proyecto",

    "Name of the school":
    "Nombre del centro",

    "Name of the synapse":
    "Nombre de la sinapsis",

    "No synapse exists for the tag «<strong>{name}</strong>».":
    "No existe ninguna sinapsis para la etiqueta «<strong>{name}</strong>».",

    "Oldest articles first":
    "Los artículos más antiguos primero",

    "Orange":
    "Naranja",

    "Pink":
    "Rosa",

    "Please enter a name for the new project.":
    "Escriba un nombre para el nuevo proyecto.",

    "Please enter a name for the new school.":
    "Escriba un nombre para el nuevo centro.",

    "Please enter a name for the new synapse.":
    "Escriba un nombre para la nueva sinapsis.",

    "Please enter a name to create a new synapse for the tag.":
    "Escriba un nombre para crear una nueva sinapsis para la etiqueta.",

    "Please, log in to like or favourite posts.":
    "Por favor, entre para marcar los artículos como favoritos.",

    "Projects":
    "Proyectos",

    "Published articles":
    "Artículos publicados",

    "Published this month":
    "Publicados este mes",

    "Published this week":
    "Publicados esta semana",

    "Published this year":
    "Publicados este año",

    "Published today":
    "Publicados hoy",

    "Purple":
    "Púrpura",

    "Red":
    "Rojo",

    "Reset form":
    "Restablecer formulario",

    "Schools":
    "Centros",

    "Share by Email":
    "Compartir por correo electrónico",

    "Share on Facebook":
    "Compartir en Facebook",

    "Share on Twitter":
    "Compartir en Twitter",

    "Synapse editor":
    "Edición de sinapsis",

    "Synapses":
    "Sinapsis",

    "Table of contents":
    "Tabla de contenidos",

    "Tags":
    "Etiquetas",

    "Tangerine":
    "Mandarina",

    "Taxonomies":
    "Taxonomías",

    "Territories":
    "Territorios",

    "The comment «{name}» has been deleted.":
    "El comentario «{name}» ha sido eliminado.",

    "The form data has been successfully saved.":
    "Los datos del formulario se han guardado con éxito.",

    "The form has errors, please correct them before saving.":
    "El formulario tiene errores, corríjalos antes de guardarlos.",

    "The project «<strong>{name}</strong>» has been deleted.":
    "El proyecto «<strong>{name}</strong>» ha sido eliminado.",

    "The school «<strong>{name}</strong>» has been deleted.":
    "El centro «<strong>{name}</strong>» ha sido eliminada.",

    "The synapse «<strong>{name}</strong>» has been deleted.":
    "La sinapsi «<strong>{name}</strong>» ha sido eliminada.",

    "The tag «<strong>{name}</strong>» has been deleted.":
    "La etiqueta «<strong>{name}</strong>» ha sido eliminada.",

    "The tag «<strong>{name}</strong>» has been restored.":
    "La etiqueta «<strong>{name}</strong>» ha sido restaurada.",

    "The user «<strong>{name}</strong>» has been re-enabled.":
    "El usuario «<strong>{name}</strong>» se ha vuelto a habilitar.",

    "The user «<strong>{name}</strong>» has been successfully disabled.":
    "El usuario «<strong>{name}</strong>» ha sido desactivado con éxito.",

    "This action cannot be undone.":
    "Esta acción no puede deshacerse.",

    "To edit the tag, a synapse must be created.":
    "Para editar la etiqueta, primero debe crearse una sinapsis.",

    "Type to search":
    "Escriba para buscar",

    "Untitled block":
    "Bloque sin título",

    "Web syndication":
    "Sindicación web",

    "White":
    "Blanco",

    "Yellow":
    "Amarillo",

    "Your account has been temporarily disabled.":
    "Tu cuenta se inhabilitó temporalmente."
};
