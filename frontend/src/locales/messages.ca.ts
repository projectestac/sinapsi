// I18n messages for TypeScript

/** Translated messages */
export const I18N_MESSAGES = {
    "About":
    "Quant a",

    "Advanced search":
    "Cerca avançada",

    "All posts":
    "Tots els articles",

    "An unexpected error happened, please try again later.":
    "S'ha produït un error inesperat; torneu-ho a provar més tard.",

    "Any changes you made to the form will be lost.":
    "Els canvis que hagueu fet al formulari es perdran.",

    "Any time":
    "En qualsevol moment",

    "Author editor":
    "Edició d'autors",

    "Black":
    "Negre",

    "Blue":
    "Blau",

    "Cancel":
    "Cancel·la",

    "Catalogs":
    "Catàlegs",

    "Commented":
    "Comentats",

    "Commented articles":
    "Articles comentats",

    "Create project":
    "Crea un projecte",

    "Create school":
    "Crea un centre",

    "Create synapse":
    "Crea una sinapsi",

    "Cyan":
    "Cian",

    "Default ordering":
    "Ordenació per defecte",

    "Delete":
    "Suprimeix",

    "Delete comment":
    "Suprimeix el comentari",

    "Delete synapse":
    "Suprimeix la sinapsi",

    "Delete tag":
    "Suprimeix l'etiqueta",

    "Disable user":
    "Desactiva l'usuari",

    "Do you want to delete «<strong>{name}</strong>»?":
    "Voleu suprimir «<strong>{name}</strong>»?",

    "Do you want to delete this block?":
    "Voleu suprimir aquest bloc?",

    "Do you want to disable the user «<strong>{name}</strong>»?":
    "Voleu desactivar l'usuari «<strong>{name}</strong>»?",

    "Do you want to reset the form to its initial state?":
    "Voleu restablir el formulari al seu estat inicial?",

    "Educators":
    "Docents",

    "Everything":
    "Tots",

    "Favourited":
    "Favorits",

    "Favourited articles":
    "Articles favorits",

    "Featured":
    "Destacats",

    "Featured articles":
    "Articles destacats",

    "Gray":
    "Gris",

    "Green":
    "Verd",

    "Help":
    "Ajuda",

    "Home":
    "Inici",

    "Leave this page":
    "Surt de la pàgina",

    "Liked":
    "Votats",

    "Liked articles":
    "Articles votats",

    "Most commented":
    "Més comentats",

    "Most commented articles":
    "Articles més comentats",

    "Most recent articles first":
    "Els articles més recents primer",

    "Most relevant results first":
    "Els resultats més rellevants primer",

    "Most voted":
    "Més votats",

    "Most voted articles":
    "Articles més votats",

    "Municipalities":
    "Municipis",

    "Name of the project":
    "Nom del projecte",

    "Name of the school":
    "Nom del centre",

    "Name of the synapse":
    "Nom de la sinapsi",

    "No synapse exists for the tag «<strong>{name}</strong>».":
    "No existeix cap sinapsi per a l'etiqueta «<strong>{name}</strong>».",

    "Oldest articles first":
    "Els articles més antics primer",

    "Orange":
    "Taronja",

    "Pink":
    "Rosa",

    "Please enter a name for the new project.":
    "Introduïu un nom per al projecte nou.",

    "Please enter a name for the new school.":
    "Introduïu un nom per al nou centre.",

    "Please enter a name for the new synapse.":
    "Introduïu un nom per a la nova sinapsi.",

    "Please enter a name to create a new synapse for the tag.":
    "Introduïu un nom per crear una sinapsi per a l'etiqueta.",

    "Please, log in to like or favourite posts.":
    "Si us plau, entreu per a marcar els articles com a favorits.",

    "Projects":
    "Projectes",

    "Published articles":
    "Articles publicats",

    "Published this month":
    "Publicats aquest mes",

    "Published this week":
    "Publicats aquesta setmana",

    "Published this year":
    "Publicats aquest any",

    "Published today":
    "Publicats avui",

    "Purple":
    "Porpra",

    "Red":
    "Vermell",

    "Reset form":
    "Restableix el formulari",

    "Schools":
    "Centres",

    "Share by Email":
    "Comparteix per correu electrònic",

    "Share on Facebook":
    "Comparteix a Facebook",

    "Share on Twitter":
    "Comparteix a Twitter",

    "Synapse editor":
    "Edició de sinapsis",

    "Synapses":
    "Sinapsis",

    "Table of contents":
    "Taula de continguts",

    "Tags":
    "Etiquetes",

    "Tangerine":
    "Mandarina",

    "Taxonomies":
    "Taxonomies",

    "Territories":
    "Territoris",

    "The comment «{name}» has been deleted.":
    "S'ha suprimit el comentari «{name}».",

    "The form data has been successfully saved.":
    "S'han desat les dades del formulari.",

    "The form has errors, please correct them before saving.":
    "El formulari conté errors, corregiu-los abans de desar.",

    "The form has unsaved changes. Do you want to leave this page without saving them?":
    "El formulari té canvis sense desar. Voleu sortir d'aquesta pàgina sense desar-los?",

    "The project «<strong>{name}</strong>» has been deleted.":
    "S'ha suprimit el projecte «<strong>{name}</strong>».",

    "The school «<strong>{name}</strong>» has been deleted.":
    "S'ha suprimit el centre «<strong>{name}</strong>».",

    "The synapse «<strong>{name}</strong>» has been deleted.":
    "S'ha suprimit la sinapsi «<strong>{name}</strong>».",

    "The tag «<strong>{name}</strong>» has been deleted.":
    "S'ha suprimit l'etiqueta «<strong>{name}</strong>».",

    "The tag «<strong>{name}</strong>» has been restored.":
    "S'ha restaurat l'etiqueta «<strong> {name} </ strong>».",

    "The tag will not longer show on search results but can be restored later on.":
    "L'etiqueta no es mostrarà més als resultats de la cerca, però es pot restaurar més endavant.",

    "The user «<strong>{name}</strong>» has been re-enabled.":
    "S'ha tornat a habilitar l'usuari «<strong> {name} </ strong>». ",

    "The user «<strong>{name}</strong>» has been successfully disabled.":
    "L'usuari «<strong>{name}</strong>» s'ha inhabilitat amb èxit.",

    "The user will no longer be able to access the application. User accounts can be temporarily disabled and re-enabled again later.":
    "L'usuari ja no podrà accedir a l'aplicació. Els comptes d'usuari es poden desactivar temporalment i tornar a activar-se més tard.",

    "This action cannot be undone.":
    "Aquesta acció no es pot desfer.",

    "To edit the tag, a synapse must be created.":
    "Per a poder editar l'etiqueta, primer cal crear una sinapsi.",

    "Type to search":
    "Escriviu per cercar",

    "Untitled block":
    "Bloc sense títol",

    "Web syndication":
    "Sindicació web",

    "White":
    "Blanc",

    "Yellow":
    "Groc",

    "Your account has been temporarily disabled.":
    "El vostre compte s'ha inhabilitat temporalment."
};
